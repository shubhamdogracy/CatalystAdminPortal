// ============================================================
// MENTOR LAYOUT — Shell wrapping all mentor pages
// ============================================================
...
const notifTypeConfig = {
  slot_booked:          { icon: '📅', color: '#0d9488', bg: '#f0fdfa' },
  session_reminder:     { icon: '⏰', color: '#7c3aed', bg: '#ede9fe' },
  assignment_submitted: { icon: '📝', color: '#f59e0b', bg: '#fef3c7' },
  student_activity:     { icon: '👤', color: '#ef4444', bg: '#fee2e2' },
};
...
export default function MentorLayout() {
  ...
  const { mentorNotifications, markMentorNotifRead, markAllMentorNotifsRead }  = useData();
  const navigate = useNavigate();

  const unreadCount  = mentorNotifications.filter(n => !n.read).length;
  const recentNotifs = [...mentorNotifications].slice(0, 5);
  ...
  const handleViewProfile  = () => { setDropdownOpen(false); navigate('/mentor/profile'); };
  const handleLogout       = () => { setDropdownOpen(false); logout(); navigate('/'); };
  const handleViewAllNotifs = () => { setNotifOpen(false); navigate('/mentor/notifications'); };
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0 min-h-0">
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 shrink-0 gap-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-px">Mentor Portal / {pageTitle}</p>
          </div>
              ...
              {notifOpen && (
                <div className="absolute top-[calc(100%+10px)] right-0 bg-white border border-gray-200 rounded-[14px] shadow-notif w-[340px] z-[1000] overflow-hidden">
                  <div className="flex justify-between items-center px-4 py-3.5 border-b border-gray-100">
                    ...
                    {unreadCount > 0 && (
                      <button className="text-xs text-mentor-primary font-semibold" onClick={markAllMentorNotifsRead}>
                        Mark all read
                      </button>
                    )}
                  </div>
                  ...
                  <div className="max-h-[320px] overflow-y-auto">
                    {recentNotifs.length === 0 ? (
                      ...
                      recentNotifs.map(n => {
                        const cfg = notifTypeConfig[n.type] || notifTypeConfig.student_activity;
                        return (
                          <div
                            key={n.id}
                            className="flex items-start gap-2.5 px-4 py-3 border-b border-gray-50 cursor-pointer transition-colors hover:bg-gray-50"
                            style={{ background: n.read ? '#fff' : '#fafffe' }}
                            onClick={() => { if (!n.read) markMentorNotifRead(n.id); }}
                          >
                            <div
                  ...
                  <button
                    className="w-full px-4 py-3 bg-gray-50 border-t border-gray-100 text-mentor-primary text-[13px] font-semibold cursor-pointer text-center hover:bg-gray-100 transition-colors"
