...
function SubjectSection({ title, borderCls, headerCls, value, onChange }) {
...
// ── Create / Edit Mock or Diagnostic Config Modal ─────────────────────────────
function CreateSubjectModal({ onClose, onSaved, existing, defaultType }) {
  const isEdit = !!existing;

  const [name,    setName]    = useState(isEdit ? existing.name : '');
  const [type,    setType]    = useState(existing?.type || defaultType || 'mock');
...
function CreatePracticeModal({ onClose, onSaved, existing }) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-[18px] shadow-[0_20px_60px_rgba(0,0,0,0.2)] w-full max-w-lg flex flex-col" style={{ height: '85vh' }}>
        ...
        <div className="px-6 py-5 flex flex-col gap-4" style={{ flex: '1 1 0', overflowY: 'auto', minHeight: 0 }}>
          ...
          {/* Cascading: Subject → Topic → Domain */}
...
function PracticeConfigCard({ config, onEdit }) {
  return (
    <div className="bg-white rounded-[14px] border border-gray-200 p-5 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div>
...
export default function SatExamConfigsPage() {
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
        ...
        activeConfigs.length === 0 ? (
          <div className="bg-white rounded-[14px] border border-gray-200 p-12 text-center text-gray-400">
            ...
            <p className="text-sm mt-1">
              {tab === 'diagnostic'
                ? 'Diagnostic tests use the same adaptive structure as mock tests. Standard counts: English 11–11, Math 9–9 per module.'
      ...
      {showPracticeModal && (
        <CreatePracticeModal onClose={() => { setShowPracticeModal(false); setEditing(null); }} onSaved={loadAll} existing={editing} />
      )}
    </div>
  );
}
